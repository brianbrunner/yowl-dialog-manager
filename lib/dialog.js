...
Dialog.prototype.play_inner = function play_inner(context, event, cb, prevIndex, messages) {
  ...
  if (typeof messages == "undefined") {
    if (typeof this.messages === "function") {
      if (this.messages.length === 3) {
        this.messages(context, event, function(err, computed_messages) {
          this.play_inner(context, event, cb, index, computed_messages);
        }.bind(this));
        return;
      } else {
        messages = this.messages(context, event); 
      }
    } else {
  ...
  var is_last = (index == messages.length - 1);

  var response = {};

  if (typeof message === "undefined" && !this.actions) {
    cb();
    return;
  } else {
  ...
  setTimeout(function() {
    ...
    event.send(response, function(err) {
      if (err) {
        ...
        if (!is_last) {
          setTimeout(function() {
            this.play_inner(context, event, cb, index, messages);
          }.bind(this), readDelay);
        } else {
